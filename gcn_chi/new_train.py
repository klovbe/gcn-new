...
test_dataset = DataSet(0, cell_set_size[0], label=cell_y, shuffle=False)
val_dataset = DataSet(sum(cell_set_size[0:3]), cell_set_size[3], label=cell_y, shuffle=False)
...
for epoch in range(FLAGS.epochs):
    ...
    val_acc = accuracy(pred_probas, val_dataset.label[:cell_set_size[0]])
...
pred_probas = np.concatenate(pred_probas, axis=0)
test_acc = accuracy(pred_probas, test_dataset.label[sum(cell_set_size[0:3]):])
